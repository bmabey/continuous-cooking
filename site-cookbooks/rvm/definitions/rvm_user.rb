define :rvm_user, :user => nil, :home => nil, :rubies => [], :gems => {}, :packages => [], :default_ruby => nil do
  ...
  rvm_package_libs = {"libz" => "libz.a", "openssl" => "libssl.a",
  ...
  (params[:rubies] + [default_ruby]).uniq.each do |ruby_version|
...
##  end
#
  params[:gems].each do |ruby_version, gems|
